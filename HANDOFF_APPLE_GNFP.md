...
|---|---|---|---|
| **gnfp-node** | **1.1.9** | win+unix zips on GitHub `v1.1.9` (`ed47a1c`) | **No.** JS only — `node src/node.js`. |
...
0.1.3 can mint a new gnfp1 or paint **0** after a failed book fetch. 0.1.4 keeps the same address and last-known spendable.
...
## Node 1.1.9 (no Apple binary)

Already at https://github.com/rgsneddon/gnfp-node/releases/tag/v1.1.9

Law (immutable, not env):

- Time never mints. Block found = miner hash meets the book target.
- Only miners mint: coinbase 1 GNFP + 0.000000001 GNFP per proven hash. `send` never mints.
- Each accepted share is a hash tx (unconfirmed). Wallet sends confirm on the **same** miner-work block.
- Many hash txs bundle to **one confirmed row per miner** at block found (same total nanos).
- 90s retarget / live floor 14 / genesis 21.

`--print-config` must show `"version":"1.1.9"` and `"minerMintOnly":1`.

---

## Already on GitHub

- Node: https://github.com/rgsneddon/gnfp-node/releases/tag/v1.1.9
